...
class ControllerMockClassFailingWithBaseError {
    async methodThrowsBaseError() {
        throw new BaseErrorClass(200, "I am an internal custom error message", 500);
    }
}
...
describe('ResponseClass Unit Tests', () => {
    ...
    describe('send', () => {
        ...
        it('should use the catch block if something wrong happens during the execution of the controller method', async () => {
            const responseClass: ResponseClass = new ResponseClass(controllerMockClass);
            const result = await responseClass.send(resMock as unknown as core.Response, RESPONSE_STATUS_CODES.OK, 'controllerClassMethodDoesNotExist');
        ...
        it('should use the catch block if something wrong happens during the execution of the controller method and assigns general unknown message', async () => {
            ...
            const result = await responseClass.send(resMock as unknown as core.Response, RESPONSE_STATUS_CODES.OK, 'methodThrowsWeirdError');
            expect(result).toBeUndefined();
        });
        ...
        it('should use the catch block if a controlled error happens during the execution of the controller method and assigns base error message', async () => {
            ...
            const result = await responseClass.send(resMock as unknown as core.Response, RESPONSE_STATUS_CODES.OK, 'methodThrowsBaseError');
            expect(result).toBeUndefined();
        });

        it('should use send method with Response Status OK', async () => {
            const responseClass: ResponseClass = new ResponseClass(controllerMockClass);
            const result = await responseClass.send(resMock as unknown as core.Response, RESPONSE_STATUS_CODES.OK, 'controllerClassMethod');
            expect(result).toBeUndefined();
        });

        it('should use send method with Response Status CREATED', async () => {
            const responseClass: ResponseClass = new ResponseClass(controllerMockClass);
            const result = await responseClass.send(resMock as unknown as core.Response, RESPONSE_STATUS_CODES.CREATED, 'controllerClassMethod');
            expect(result).toBeUndefined()
        });

        it('should use send method with Response Status ACCEPTED', async () => {
            const responseClass: ResponseClass = new ResponseClass(controllerMockClass);
            const result = await responseClass.send(resMock as unknown as core.Response, RESPONSE_STATUS_CODES.ACCEPTED, 'controllerClassMethod');
            expect(result).toBeUndefined()
        });

        it('should use send method with Response Status NO_CONTENT', async () => {
            const responseClass: ResponseClass = new ResponseClass(controllerMockClass);
            const result = await responseClass.send(resMock as unknown as core.Response, RESPONSE_STATUS_CODES.NO_CONTENT, 'controllerClassMethod');
            expect(result).toBeUndefined()
        });

        it('should use send method with Response Status BAD_REQUEST', async () => {
            const responseClass: ResponseClass = new ResponseClass(controllerMockClass);
            const result = await responseClass.send(resMock as unknown as core.Response, RESPONSE_STATUS_CODES.BAD_REQUEST, 'controllerClassMethod');
            expect(result).toBeUndefined()
        });

        it('should use send method with Response Status UNAUTHORIZED', async () => {
            const responseClass: ResponseClass = new ResponseClass(controllerMockClass);
            const result = await responseClass.send(resMock as unknown as core.Response, RESPONSE_STATUS_CODES.UNAUTHORIZED, 'controllerClassMethod');
            expect(result).toBeUndefined()
        });

        it('should use send method with Response Status FORBIDDEN', async () => {
            const responseClass: ResponseClass = new ResponseClass(controllerMockClass);
            const result = await responseClass.send(resMock as unknown as core.Response, RESPONSE_STATUS_CODES.FORBIDDEN, 'controllerClassMethod');
            expect(result).toBeUndefined()
        });

        it('should use send method with Response Status NOT_FOUND', async () => {
            const responseClass: ResponseClass = new ResponseClass(controllerMockClass);
            const result = await responseClass.send(resMock as unknown as core.Response, RESPONSE_STATUS_CODES.NOT_FOUND, 'controllerClassMethod');
            expect(result).toBeUndefined()
        });
        
        it('should use send method with Response Status REQUEST_TIMEOUT', async () => {
            const responseClass: ResponseClass = new ResponseClass(controllerMockClass);
            const result = await responseClass.send(resMock as unknown as core.Response, RESPONSE_STATUS_CODES.REQUEST_TIMEOUT, 'controllerClassMethod');
            expect(result).toBeUndefined()
        });

        it('should use send method with Response Status INTERNAL_SERVER_ERROR', async () => {
            const responseClass: ResponseClass = new ResponseClass(controllerMockClass);
            const result = await responseClass.send(resMock as unknown as core.Response, RESPONSE_STATUS_CODES.INTERNAL_SERVER_ERROR, 'controllerClassMethod');
            expect(result).toBeUndefined()
        });
